...
import {
  Sheet, Paper, Display, Label, Pill, ArrowLink,
  HandleField, LetterField, Icon,
} from '../parts.jsx'
import { Dots, Sparkle } from '../art.jsx'
import { normHandle, validHandle, atHandle, dateline } from '../data.js'
import { getState, patch } from '../store.js'

const MIN_BODY = 60    // characters. Below this it is a comment, not a letter.
const MAX_BODY = 320
...
export default function Write({ to: prefill, go, back }) {
  ...
  // Somebody who tapped "write one to @them" on a letter already answered one.
  const [step, setStep] = useState(() => (prefill ? 1 : 0))
  const first = useRef(true)

  const h = normHandle(to)
  const ok = [validHandle(h), body.trim().length >= MIN_BODY]
    ...
    <Sheet onClose={back} tall labelledBy="wl-write-h">
      <div className="wl-sheet-in wl-write">
        ...
        <div className="wl-write-foot">
          {step === 1 && (
            <ArrowLink tone="quiet" size="s" onClick={() => setStep(0)}>a different name</ArrowLink>
          )}
