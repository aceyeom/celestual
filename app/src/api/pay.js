// pay.js — the paywall for CELESTUAL.
//
...
// through a Supabase Edge Function (`celestual-checkout`) so secret keys stay
...
//   1. Deploy supabase/functions/celestual-checkout (creates a session per provider).
...
const STORE = 'celestual:entitlement'
...
export async function startCheckout(provider) {
  ...
  const { data, error } = await supabase.functions.invoke('celestual-checkout', {
