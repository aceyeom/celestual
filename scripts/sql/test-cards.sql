...
begin
  begin
    ...
    values ('x', 'x', 'berkeley', 'https://elsewhere.example/x');
...
select cd_ok('a step lands',
  (wall_card_step('a', 'joined', 'berkeley')->>'logged')::boolean);
...
insert into wall_scans (source_code, campus) values
  ('a', 'berkeley'), ('b', 'berkeley'), ('b', 'berkeley'),
  ('flyer-a', 'berkeley');
...
begin
  ...
  insert into wall_letters (target_handle, body, author_id, campus, source_code, status)
    values ('someone.oncard', 'forty words, or fewer.', u, 'berkeley', 'b', 'live');
end $$;

insert into wall_waitlist (handle, campus, source_code)
values ('nobody.wrote.yet', 'berkeley', 'b') on conflict do nothing;
...
select cd_ok('the card with a proof against it is first',
  (celestual_desk_cards()#>>'{rows,0,code}') = 'a');
...
select cd_ok('a card can be named and placed',
  (celestual_desk_card_set('c', 'the quiet one', 'moffitt, 4th floor')->>'ok')::boolean);
select cd_ok('the name and the place are kept',
  (select label = 'the quiet one' and place = 'moffitt, 4th floor' from wall_cards where code = 'c'));
...
select celestual_desk_card_set('c', 'the quiet one');
select cd_ok('a null leaves the place alone',
  (select place from wall_cards where code = 'c') = 'moffitt, 4th floor');

select cd_ok('an empty place is accepted',
  (celestual_desk_card_set('c', null, '')->>'ok')::boolean);
select cd_ok('an empty string clears the place',
  (select place is null from wall_cards where code = 'c'));

select cd_ok('a card can be taken out of circulation',
  (celestual_desk_card_set('c', null, null, false)->>'ok')::boolean);
select cd_ok('a card out of circulation is still a row on the desk',
  (select not is_active from wall_cards where code = 'c')
    and jsonb_array_length(celestual_desk_cards()->'rows') = 5);
select cd_ok('a code that is not a card is not found',
  (celestual_desk_card_set('z', 'nothing')->>'error') = 'not_found');
