...
| Veil | `.wl-veil` | the wall's masthead, over the hive rather than above it: the title, the one line and `view the wall` on a scrim that is deep under the words, gone where they are not, and is itself the way in. Nothing else is on the screen under it: no pill, no foot, no glyphs in the bar. It opens as a circle from the tap. Once per tab |
