...
| `wl-sheen`, `wl-nudge` | the letter going up (`.wl-posted`): the light passing over the paper while it is read, and the neighbours nudged aside as the name lands. The card's own closing into the disc is a measured transform, written by the screen |
