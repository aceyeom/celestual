...
| the veil (`.wl-veil`) | the wall's masthead laid over its greyed hive, lifted once per tab, from the tap: 1050ms on a shallow ease out, the grey and the type opened together as a circle from where the veil was touched, a hairline ring on its edge, while the field comes up to full light and the lens blooms inside it (`wall/Hive.jsx`). Then the bar's glyphs and the pill rise in, 620 to 700ms, a beat apart. The ear does not move. Under reduced motion it goes without travelling |
...
| Sheet | `.wl-sheet` | rises off the bottom edge over a wall that stays mounted, dimmed and slightly out of focus behind it. A centred dialog at 900px. Given an `origin` it opens out of that point instead, everything on it already there (`.wl-sheet-lift`) |
| Deck | `.wl-deck`, `Deck` in `wall/screens/Letter.jsx` | the letters under one name as cards on one track: the one being read at the middle, the neighbours' edges either side, a swipe that moves the cards, dots under them, chevrons in the dialog's band on a wide screen |
