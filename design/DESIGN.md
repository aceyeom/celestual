...
| Bookmarks | `.wl-seg`, `Segmented` | two or three words on a rail, and the one that is open sitting on the rail the way a bookmark sits on a page: a soft ground rounded at the head and square at the foot, and a lit spine along the hairline under it. The rail is the TOP EDGE of the thing the tab changes, so tab and content read as one object — the composer's field hangs under it, the look panel's grid hangs under it. One element sliding on the sheet's own curve. A radio group to a keyboard, a tablist over the look panel. The composer's first question, `instagram` or `anything else`. It was a filled capsule with an inset ring and a glyph beside each word: a second box standing over a bare baseline field, a lit ground spent on a choice while the act at the foot is the screen's one bright thing, and an icon set the product does not have |
...
| The answer in the field's place | `.wl-field.is-settled`, `Addressed`, on the wall's composer. The same answer, standing where the handle was typed rather than under it: the same measure, the same rule, the same left edge the painted `@` held, and no plate and no frame, because it is not a second object arriving under the field — it IS the field, answered. The way out of it is the close mark at its end and not an arrow: an arrow says the row is the way on, and the way on is the capsule at the foot of the sheet. While the lookup is out the field stays where it is, one light travels its own rule (`.wl-field.is-busy`), and a sentence under it says what is being waited on |
