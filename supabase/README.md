...
- `migrations/0050_the_letter_goes_up_first.sql`: **the letter goes up
  first.** The screen's hold is gone: a letter the classifier is unsure of is
  written at `live` with `moderation.flagged` and a person reads it at the
  desk while it stands. Adds `wall_mine(token)`, the caller's own letters and
  whose hand took one down (`down_by`: screen, desk, report, shut, lapsed),
  which is what the wall's notice to a writer draws from; teaches
  `celestual_desk_letters` the status `'flagged'`; renames the standing
  `celestual_desk_overview` to `celestual_desk_overview_0039` and puts a
  wrapper under the old name that adds `counts.letters_flagged`. Nothing about
  `wall_letters` changes. **Tested by `scripts/sql/test-flagged.sql`, 25
  assertions.**
