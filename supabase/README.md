...
- `migrations/0061_the_prints_thin_out.sql`: **the prints thin out.** The
  ...
  blue to rose, cobalt to ice, orange / teal and red / green to ember. Each
  moved row's look is copied first into `wall_look_backup_0061`, service role
  only, first copy kept. No schema change: `wall_look_clean` and the
  constraint over it check a look's shape and are left as 0055 left them.
  Apply after the app that draws the five as their new colours (looks.js
  `RETIRED`) is live. Re-runnable. Undo:
  `update wall_letters l set look = b.look from wall_look_backup_0061 b where b.id = l.id;`
- `migrations/0060_the_resolver_canary.sql`: **the resolver checks itself,
  once a day.** A provider failure draws nothing, so an Apify outage used to
  be a quiet day. `resolver_canary_runs` keeps one row per check: when, from
  the schedule or the desk, whether it passed, and when it did not, Apify's
  own status and words (`ok`, `shape`, `missing`, `unclear`, `timeout`,
  `refused`, `off` or `error`), the seconds, the tries and whether the face
  would have downloaded; ninety days of them. `resolver_canary_due()` says
  whether a check is owed (a day after a pass, three hours after a failure,
  never with the resolver switched off); `resolver_canary_begin` opens a row
  or says why not, and runs one for the service key's caller whenever it asks,
  one at a time and a minute apart; `resolver_canary_finish` closes it, taking
  `detail` key by key; `resolver_canary_status(n)` is what the desk reads.
  `celestual_desk_overview()` keeps 0050's body word for word and carries
  `canary`. The job `celestual-resolver-canary` ticks at minute 41 of every
  hour and posts to `celestual-resolve` through pg_net only when a check is
  owed, with two minutes to answer. All service role only. **Tested by
  `scripts/sql/test-canary.sql`, 39 assertions.** Runbook:
  docs/HANDLE-RESOLVER.md, 12.
- `migrations/0059_the_hearts_the_wall_began_with.sql`: **the hearts the wall
  began with.** Every letter up when it first runs is given a number of hearts
  to start from, once, in `wall_letters.hearts_seed` (0 to 30): nought 35 times
  in 100, otherwise the size of a normal draw a third of the ceiling wide,
  never more than 12 on the campus wall and 30 on the wall at the root
  (`wall_hearts_seed_draw`). A `celestual_settings` row says it has run, so a
  second run, and a database built from these files, gives nothing to any
  letter written since; every later letter starts from nought.
  `wall_letters_for`, `wall_letter` (0055), `wall_mine` (0056) and
  `wall_heart` (0044) are re-emitted word for word with the count as the seed
  and the pressed hearts added; nothing returns the seed on its own, and
  whether a reader hearted a letter is still their own press. What each letter
  was given is kept in `wall_hearts_seed_0059`, service role only. This is a
  padded count, which design/VOICE.md ("Truth, exactly") rules out; it was asked
  for, and it comes out with `update wall_letters set hearts_seed = 0;` and
  goes back from the record. **Tested by `scripts/sql/test-hearts-seed.sql`, 40
  assertions**, including a rolled back second and first run of the file.
  `test-hearts.sql`, `test-reading-room.sql` and `test-wall.sql` had held the
  read gate to the campus since before 0057 and were failing; their outsider
  reads now, and the refusals are asserted on a session with nothing proved.
- `migrations/0055_the_letter_has_a_look.sql` (**applied 20 September 2026**,
  through the Supabase MCP): **the letter has a look, and a name can be
  anything.** `wall_letters` gains `look`, one jsonb of up to three slugs,
  `{theme, tint, face}`, null for the plain paper, held by
  `wall_letters_look_ck` to what `wall_look_clean` answers, so a row can never
  hold a string, a colour, a link or a fourth key. The catalogue of what a
  slug draws is the browser's (`app/src/wall/looks.js`), so a new look is a
  client change and a slug an old build does not know draws the plain paper.
  `wall_index` appends `look`, the newest letter's under the key, which is the
  paper the name's disc draws; `wall_write` gains an eleven argument overload
  with `p_look` (the ten and the eight argument forms stay and call it);
  `wall_letters_for`, `wall_letter`, `wall_mine`, `wall_search` and
  `celestual_desk_letters` carry it. The name is widened with it: `wall_fold`
  admits any letter, `wall_name_key` needs one character, `wall_name_clean`
  admits one to thirty characters, five words and digits and refuses only the
  characters that make a string a handle, a link or a command, and the two
  checks on `wall_letters` are re-emitted to match. What the wall discloses
  does not change: a look is a choice from a menu every writer shares, and a
  takedown takes it with the letter (docs/WALL-FEATURES.md records the
  ruling). **Tested by `scripts/sql/test-looks.sql`**, and four assertions in
  `test-names.sql` turned for the wider name.
- `migrations/0054_the_search_hears_a_name.sql` (**applied 20 September 2026**,
  through the Supabase MCP): **the search hears a name.** `wall_search`
  re-emitted over `wall_index` and nothing else: the handle (exact, prefix,
  contains), the handle with its dots out, and the
...
- `migrations/0053_a_letter_to_a_first_name.sql` (**applied 20 September 2026**,
  through the Supabase MCP): **a letter to a first name.** `wall_letters`
  gains `target_kind` and `target_name`; a letter to a first name is keyed by
  a tilde and the folded name (`~sofia`,
